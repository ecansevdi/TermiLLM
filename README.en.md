...
- Requests go to the `chat.completions` streaming endpoint. The default effort is `auto` (internal `None`): TermiLLM sends no explicit reasoning effort, and the hint is `Efor: auto`. `auto` is not `none`. `none` is an explicit request to turn reasoning off, applied only when the model supports it. `minimal` / `low` / `medium` / `high` / `xhigh` / `max` are explicit levels mapped by the provider adapter. Ctrl+P can return to `auto`. The choice, including `auto`, survives a provider switch. Claude on `api.anthropic.com` uses the native Messages API (`output_config.effort`) only when a level is selected.
